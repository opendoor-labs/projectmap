...
To allow multiple users to edit the same R file and perform version control, the **projectmap** package provides two functions: _branch()_ and _merge_branch()_. 

First, a user that wants to edit a local version of a file rather than the master file, the user should call the _branch()_ function. Say, a user wants to edit "Project Master.R" separately from the master version, the user should type:
```{r, echo = T, eval = F}
library(projectmap)
branch("Project Master.R")
```
in an RStudio console. If the working directory is not a root folder for a **projectmap** package, it will prompt the user to select the appropriate directory. It will then detect the user's username on the computer and create a directory "Branches/username" with the "Project Master_username.R" (where "username" is replaced with the user's actual username) in that folder. The user should then be able to edit this file without fear of disrupting the master file. All output created from the cloned file that saved with the _save_file()_ function will be placed in the "Output/Branches" subfolder by default so as not to overwrite output created from master files.

Once the user is done editing the local copy of "Project Master.R" and wants to merge the changes with the master file, the user should call the _merge_branch()_ function:
```{r, echo = T, eval = F}
merge_branch("Project Master.R")
```
The function will automatically detect the user's username and look in the "Branches/username" folder for their cloned "Project Master.R" file. The function will then process the two versions and display the changes in the RStudio window for approval. The RStudio console will prompt the user to enter "y" (to commit the changes) or "n" (to reject the changes) and a commit message. After these are entered, the master file is overwritten with the users cloned file and a log is made in "Logs/Merges/Project Master" with a file "username_datetime.hmtl" (where "username" is replaced with the user's actual username and "datetime" is replaced with the date and time of the merge) for records. The commit message is saved as a "Commit Message.txt" in the folder with the same name as "username_datetime.hmtl".
